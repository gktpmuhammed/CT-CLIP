...
parser.add_argument(
    ...
    help='model_path',
    required=False,
    default='/home/muhammedg/CT-CLIP/text_classifier/checkpoints/models/RadBertClassifier.pth') 
...
parser.add_argument(
    ...
    required=False,
    default="./output/")
# Parse args inside main() to avoid running argument parsing on worker imports
...
def main():
    ...
    # Create save directory robustly; if exists, make a unique one
    ...
    if device.type == 'cuda':
        ...
        gpu_name = torch.cuda.get_device_name(0) if n_gpu > 0 else 'Unknown GPU'
        print("Number of GPU available:{} --> {} \n".format(n_gpu, gpu_name))
...
if __name__ == '__main__':
    # Use 'fork' start method where possible to avoid re-importing the main module in workers
    try:
        import torch.multiprocessing as mp
        mp.set_start_method('fork', force=True)
    except Exception:
        # If setting start method fails (already set or not supported), ignore
        pass
    main()
